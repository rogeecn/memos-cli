...
```bash
git clone https://github.com/rogeecn/memos-cli.git
cd memos-mcp
uv sync
uv run memos-mcp
```
...
可以通过环境变量，或在当前工作目录放置 `.env` 文件来配置。Go CLI 会自动读取当前目录 `.env`，并遵循“系统环境变量优先，缺失时回退到 `.env`”的规则：
...
仓库内新增了一个基于 Go 的命令行工具实现，用于直接调用 Memos HTTP API，不依赖 Python 运行时。

本地运行：

```bash
go run ./cmd/memos --help
```
...
- Python MCP 入口定义在 [`pyproject.toml`](pyproject.toml)
- Python MCP 服务实现位于 [`src/memos_cmp/server.py`](src/memos_cmp/server.py)
- Python 包版本定义位于 [`src/memos_cmp/__init__.py`](src/memos_cmp/__init__.py)
- Go CLI 入口位于 [`cmd/memos/main.go`](cmd/memos/main.go)
- Go CLI 命令位于 [`internal/cli/root.go`](internal/cli/root.go)
